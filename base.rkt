...
(provide (struct-out posn)
         ...
         TIMEOUT-PADRAO
         Q-ALTURA Q-LARGURA PONTOS-NIVEL PONTOS-LINHA
         TIMEOUT-PADRAO
         BLANK
         CORES
...
#;
(define (fun-for-linha linha)
  (cond
    [(empty? linha) ...]
    [else
     (... (first linha)
          (fun-for-linha (rest linha)))]))

;; Template de função para o tipo campo
#;
(define (fun-for-campo campo)
  (cond
    [(empty? campo) ...]
    [else
     (... (fun-for-linha (first campo))
          (fun-for-campo (rest campo)))]))

(struct posn (lin col) #:transparent)
;; Posn - representa a posição de um quadrado no campo
;;   lin : Integer - é o número da linha começando com 0
;;   col : Integer - é o número da coluna començando com 0
;;
;; Template de função para o tipo posn
#;
(define (fun-for-posn p)
  (... (posn-lin p)
       (posn-col p)))

(struct tetramino (tipo rot pos cor) #:transparent)
;; Tetramino - representa uma tetraminó no jogo tetris
;;   tipo : List - é uma lista com todas as representações (rotações) de um
;;                 tetraminó. Cada representação consiste de uma lista de
;;                 listas com o valor 1 nas posições ocupadas pelo tetraminó.
;;                 Veja as constantes definidas no arquivo tetra-tipos.rkt e
;;                 as constantes I J L O S T Z neste arquivo
;;   rot : Natural - é a rotação, que é utilizado como indíce da lista tipo
;;                   para selecionar a representação desta rotação
...
#;
(define (fun-for-tetramino t)
  (... (fun-for-list (tetramino-tipo t))
       (tetramino-rot t)
       (fun-for-pos (tetramino-pos t))
       (tetramino-cor t)))

(struct tetris (campo largura altura tetra proximos timeout pontos) #:transparent)
;; Tetris - representa um jogo tetris
;;   campo   : Campo - é o campo do jogo (veja a definição do tipo campo)
;;   largura : Natural - e altura são as dimensões do campo
;;   tetra   : Natural - é o tetraminó que está caindo
;;   proximos: Stream(Tetramino) - é um stream com os próximos tetraminós
;;   timeout : Natural - é um contador regressivo de ticks que controla o
;;                       automovimento
;;   pontos  : Natural - é o número de pontos do jogar na rodada
;;
;; Template de função para o tipo tetris
#;
(define (fun-for-tetris jogo)
  (... (fun-for-campo (tetris-campo jogo))
       (tetris-largura jogo)
       (tetris-altura jogo)
       (fun-for-tetramino (tetris-tetra jogo))
...
;; Divisor para se identificar o nível atual
;;ex: total de pontos = 500
;;    PONTOS-NIVEL = 100  
;;    nivel atual = 500 / 100 = 5 
(define PONTOS-NIVEL 100)

;; Pontos que o jogodor ganha a cada linha eliminada
(define PONTOS-LINHA 50)
...
(define (make-tetris largura altura tetras timeout pontos)
